...
def parse_dec_file(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            line = line.strip()
            if line.startswith('<balloon_msg>'):
                current_section = 'balloon_msg'
                data[current_section] = {}
            elif line.startswith('</balloon_msg>'):
                current_section = None
        ...
        if match:
            ...
            if msg_id in translations:
                ...
                if 'cont' in translation:
                    line = re.sub(r'cont="[^"]*"', f'cont="{translation["cont"]}"', line)
            else:
...
def translate_file(input_file, translation_data, output_file, untranslated_file):
    ...
    for line in lines:
        if line.strip().startswith('<balloon_message>'):
            ...
            section_lines = []
            section_opened = False
        elif line.strip().startswith('<balloon_msg>'):
            ...
            section_lines = []
            section_opened = False
        elif line.strip().startswith('</balloon_message>'):
            section_lines.append(line)
            translated_lines.extend(translate_section_lines(section_lines, translation_data.get(current_section, {}), current_section, untranslated_lines))
            current_section = None
            section_lines = []
