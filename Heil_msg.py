    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            if line.startswith('<season_msg>'):
                ...
                data[current_section] = defaultdict(dict)
            elif line.startswith('</season_msg>') or line.startswith('</clue_rank>') or line.startswith('</change_group>'):
                current_section = None
            elif line.startswith('<msg'):
                match = re.search(r'id="(\d+)"\s+desc="([^"]*)"', line)
                if match:
...
def translate_file(input_file, translation_data, output_file, untranslated_file):
    ...
    for line in lines:
        if line.strip().startswith('<Heil_message>'):
            ...
            section_lines = []
            section_opened = False
        elif line.strip().startswith('<season_msg>') or line.strip().startswith('<clue_rank>') or line.strip().startswith('<change_group>'):
