    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            if line.startswith('<season_msg>'):
                ...
                data[current_section] = defaultdict(dict)
            elif line.startswith('<category_name_msg>'):
                current_section = 'category_name_msg'
                data[current_section] = defaultdict(dict)
            elif line.startswith('<mastery_name_msg>'):
                current_section = 'mastery_name_msg'
                data[current_section] = defaultdict(dict)
            elif line.startswith('<mastery_desc_msg>'):
                current_section = 'mastery_desc_msg'
                data[current_section] = defaultdict(dict)
            elif line.startswith('</season_msg>') or line.startswith('</clue_rank>') or line.startswith('</change_group>') or \
                 line.startswith('</category_name_msg>') or line.startswith('</mastery_name_msg>') or line.startswith('</mastery_desc_msg>'):
                current_section = None
...
def translate_file(input_file, translation_data, output_file, untranslated_file):
    ...
    for line in lines:
        if line.strip().startswith('<meritsystem_message>'):
            ...
            section_lines = []
            section_opened = False
        elif line.strip().startswith('<season_msg>') or line.strip().startswith('<clue_rank>') or line.strip().startswith('<change_group>') or \
             line.strip().startswith('<category_name_msg>') or line.strip().startswith('<mastery_name_msg>') or line.strip().startswith('<mastery_desc_msg>'):
            ...
            section_lines = []
            section_opened = False
        elif line.strip().startswith('</meritsystem_message>'):
            section_lines.append(line)
            translated_lines.extend(translate_section_lines(section_lines, translation_data[current_section], current_section, untranslated_lines))
            current_section = None
            section_lines = []
...
file_name_dec = 'meritsystem_msg.dec'
