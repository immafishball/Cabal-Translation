...
def parse_dec_file(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            line = line.strip()
            if line.startswith('<extra_obj_msg>'):
                current_section = 'extra_obj_msg'
                data[current_section] = defaultdict(dict)
            elif line.startswith('</extra_obj_msg>'):
                current_section = None
            elif line.startswith('<msg'):
                match = re.search(r'id="([^"]+)"\s+cont="([^"]*)"', line)
                if match:
                    msg_id, cont = match.groups()
                    data['extra_obj_msg'][msg_id] = {'cont': cont}
    return data
...
def translate_file(input_file, translation_data, output_file, untranslated_file):
    ...
    for line in lines:
        if line.strip().startswith('<cabal_message>'):
            ...
            translated_lines.extend(translate_section_lines(section_lines, translation_data[current_section], untranslated_lines))
            current_section = None
            section_lines = []
            ...
            current_section = None
            section_lines = []
        elif line.strip().startswith('<extra_obj_msg>'):
            section_lines.append(line)
            translated_lines.extend(translate_section_lines(section_lines, translation_data[current_section], untranslated_lines))
            current_section = 'extra_obj_msg'
            section_lines = []
...
file_name_dec = 'extra_obj_msg.dec'
