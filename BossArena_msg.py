...
def parse_dec_file(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            line = line.strip()
            if line.startswith('<BossArena_msg>'):
                current_section = 'BossArena_msg'
                data[current_section] = {}
            elif line.startswith('</BossArena_msg>'):
                current_section = None
...
def translate_file(input_file, translation_data, output_file, untranslated_file):
    ...
    for line in lines:
        if line.strip().startswith('<cabal_BossArena_message>'):
            ...
            section_lines = []
            section_opened = False
        elif line.strip().startswith('<BossArena_msg>'):
            ...
            section_lines = []
            section_opened = False
        elif line.strip().startswith('</cabal_BossArena_message>'):
            section_lines.append(line)
            translated_lines.extend(translate_section_lines(section_lines, translation_data.get(current_section, {}), current_section, untranslated_lines))
            current_section = None
            section_lines = []
...
file_name_dec = 'BossArena_msg.dec'
